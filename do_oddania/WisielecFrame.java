...
public class WisielecFrame extends javax.swing.JFrame implements Runnable {
    GUIModel model; // asocjacja do modelu

    // Elementy globalne
    private javax.swing.JLabel picLabel;
    private javax.swing.JPanel mainPanel;
    private javax.swing.JTextField textField;
    private final LinkedList<JButton> btnCollection = new LinkedList<>(); // Przechodzimy po guziki jeden po drugim

    public WisielecFrame() {
        this.model = new GUIModel();
        chooseLevel(); // Pozwala wybrać poziom trudności
    }

    public void run() {
        initComponents(); // Tworzy elemnty GUI
        setVisible(true); // Ustawia widocznośc okna
        setDefaultCloseOperation(javax.swing.WindowConstants.EXIT_ON_CLOSE);
        setBackground(new java.awt.Color(30, 30, 30));
        setResizable(false); // Stała wielkość okna
    }
    ...
    private void initComponents() {
        ...
        //Obsluga guzikow, słuchacze

        ActionListener btnListener = e -> { // wyrażenie lambda
            for (JButton btn : btnCollection) {
                if (e.getSource() == btn) { // jeżeli guzik jest kliknięty
                    char c = btn.getText().toLowerCase().charAt(0); // przypisz do zmiennej literę guziaka
                    addLetter(c); // Dodaj literę do kolekcji
                    btn.setEnabled(false); // Zablokuj guzik
                    setLives(c); // Zmień ilośc żyć
                }
            }
            displayString(checkWord()); //Zmiena tekstu w komponencie, ustawienie dasza albo litery zależnie od tego co zostało kliknięte
            setPicture(picLabel); // Zmienia obraz wisielca
            checkGameState(); // Sprawczy czy nastąpiła wygrana czy przegrana
        };

        for (JButton btn : btnCollection) {
            btn.addActionListener(btnListener); // Doadanie słuchacyz do guzików
        }

        btnRestart.addActionListener(e -> reset()); // Obsługa guzika restartu

        // Obsluga przycisku zgadywania hasla
        btnGuessWord.addActionListener(e -> { // Obsługa guziaka do zganięcia całego słowa
            String guessedWord = JOptionPane.showInputDialog(mainPanel, "Guess word");
            guessCheck(guessedWord);
        });

        btnAddWord.addActionListener(e -> model.openAddWordPage()); // Obsługa guziaka do otwarcia nowego okna do wprowadzania słów


    }

    private String checkWord() { // Zbuduj słowo do wyświetlenia
        return model.checkWord();
    }

    private void displayString(String word) { // Zmiana tekstu w komponencie teksotwym
        textField.setText(word);
    }

    private boolean isWinner() { // Sprawdź czy słowo zostało zgadnięte
        return model.isWinner();
    }

    private boolean isLooser() { // Sprawdź czy żcyia są równe 0
        return model.isLooser();
    }

    private String setWord() { // Zwróć tyle daszy ile słowo ma liter
        return model.setWord();
    }

    private void setPicture(JLabel label) { // Ustawia obrazek do wyświetlenia
        try{
    ...
    private void setLives(char c) { // Sprawdź czy zganięta literka jest poprawna i obsłuż ilość żyć
        model.setLives(c);
    }

    private void addLetter(Character letter) { // Dodanie literki do kolekcji literek
        model.addLetter(letter);
    }

    private void disposeWindow() { // Funkcja wyłączenia okna po zakończenu gry
        this.setVisible(false);
        this.dispose();
        System.exit(0); // Wyłącz program z kodem 0
    }

    private void guessCheck(String word) { // Sprawdź czy zganięte słowo jest poprawne *guzik Guess Word*
    ...
    private void chooseLevel() { // Funckja wyświetla okno dialogowe i urchamia grę z odpowiednim poziomem trudności
    ...
    private void reset() { // Restart rozgrywki
        chooseLevel();
        model.reset();
    ...
    private void checkGameState() { // Sprawdza czy nastąpiło zwycięstwo albo przegrana
        if (isWinner()) { // Czy słowo zostało zgadnięte
            JOptionPane.showMessageDialog(mainPanel,
                    "You were right. The word was: " + model.getRandomWord(),
                    "You win! Congratulations!",
                    JOptionPane.WARNING_MESSAGE);
            disposeWindow();
        } else if (isLooser()) { // Czy życia są równe 0
            JOptionPane.showMessageDialog(mainPanel,
